...
class Cache:
    ...
    def add(self, entry, format, data, ttl_min, ttl_sec=0):
        if entry in self.db:
            ...
            self._log(f'Entry "{entry}" updated with "{format}" data in cache.')
        else:
            self.db[entry] = {
            ...
            self._log(f'Entry "{entry}" created with "{format}" data in cache.')
